...
int main(int argc, char *argv[])
{
	...
	if (is_32bit) {
		...
		for (index = 0; index < elf_header32.e_shnum; index++)
		{
			...
			print_Section_Info_32bits(index, section_header32);
		}
	}
	else
	{
		...
		for (index = 0; index < elf_header64.e_shnum; index++)
		{
			...
			print_Section_Info_64bits(index, section_header64);
		}
	}
	printKeyToFlags();
	fclose(file);
...
void print_Section_Info_32bits(int index, Elf32_Shdr section_header)
{
	...
	printf("  [%2d] %-17d %-15s %08x %06x %06x %02x %s %2d %2d %2d \n",
		index, section_header.sh_name, getSectionTypeName(section_header.sh_type),
...
{
	const char *flags = getSectionFlags(section_header.sh_flags);
	printf("  [%2d] %-17d %-15s %08lx %06lx %06lx %02lx %s %2d %2d %2ld \n",
		index, section_header.sh_name, getSectionTypeName(section_header.sh_type),
		section_header.sh_addr, section_header.sh_offset,
		section_header.sh_size, section_header.sh_entsize, flags,
		section_header.sh_link, section_header.sh_info, section_header.sh_addralign);
